...
export const TopSneakerProductsContext = createContext();
...
import Top_sneak from './Top_sneak';
...
export default function Top_sneakers() {
    ...
    if (products.length > 0) {
      products.map((pro) => {
        if (pro.category === 'Shoes' && pro.price >= 5000) {
            sneaker.push(pro);
        }
      })
    }
  ...
  return (
    <Box my={10} maxW={{'2xl' : '80%', xl : '95%', lg : '100%', base: '97%'}} mx={'auto'} className='md:mb-0 mb-0 bg-white rounded-lg'>
        <Box className='bg-white py-3 rounded-t-lg px-3 text-white'>
            <Box className="flex justify-between items-center">
                <Heading fontWeight={500} fontSize={{md:20, base: 18}} className='text-xl text-gray-800'>Top Shoes Collection</Heading>
              ...
              currentPost.map((product) => (
                product.deal === 'great' ? (
                  <Box key={product._id} p={1} shadow={'md'}>
                    <TopSneakerProductsContext.Provider value={product}>
                        <Top_sneak product={product}/>
                    </TopSneakerProductsContext.Provider>
                  </Box>
                ) : ''
              ))
          }
