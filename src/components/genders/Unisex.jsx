...
export const UinisexProductsContext = createContext();
...
export default function Unisex() {
  ...
  const combinedProducts = products.filter(
    (pro) => ['unisex', 'male', 'female'].includes(pro.gender) && pro.deal === 'great'
  );

  const startIndex = currentPage * postPerPage;
  const endIndex = startIndex - postPerPage;
  const currentPost = combinedProducts.slice(endIndex, startIndex);

  return (
    <Box mt={7} maxW={{ '2xl': '80%', xl: '95%', lg: '100%', base: '97%' }} mx={'auto'} className='md:mb-0 mb-0 bg-white rounded-lg'>
      <Box borderBottomWidth={'1px'} borderBottom={'solid gray.300'} className='bg-white py-3 rounded-t-lg px-3 text-black'>
        <Box className="flex justify-between items-center" color={'gray.800'}>
          <Heading fontWeight={500} fontSize={{ md: 20, base: 18 }} className='text-xl'>Unisex</Heading>
        ...
        <Slider {...settings}>
          {
            currentPost.map((product) => (
              <Box key={product._id} p={1} shadow={'md'}>
                <UinisexProductsContext.Provider value={product}>
                  <Uni product={product} />
                </UinisexProductsContext.Provider>
              </Box>
            ))
          }
        </Slider>
