...
export const JeansContext = createContext();

const Jeans = React.lazy(() => import('../../components/mensCategory/Jeans'));

// Main Component
export default function Jeans_page() {
  const clothingLinks = [
  ...
  const filteredProducts = products.filter((product) => {
    ...
    filter === 'All' ? product.gender === 'male' && product.category === 'Jeans' : product.category === filter && product.gender === 'male';
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <JeansContext.Provider value={product}>
                        <Jeans key={product._id} product={product} />
