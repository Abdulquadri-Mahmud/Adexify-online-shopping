...
export const SocksContext = createContext();

const Socks = React.lazy(() => import('../../components/mensCategory/Socks'));

// Main Component
export default function Socks_page() {
  const clothingLinks = [
  ...
  // const { category } = useParams();  // Gets 'mens-socks' from the URL
  ...
  const filteredProducts = products.filter((product) => {
    ...
    filter === 'All' ? product.gender === 'male' && product.category === 'Socks' : product.category === filter && product.gender === 'male';
          ...
          <Flex gap={5} flexWrap="wrap">
            ...
            <Box flex={1} bg="white" rounded="md">
              <Flex justifyContent="space-between" alignItems="center" className="mb-4 border-b pb-5">
                ...
                <Select
                  width={{md:"200px", base: '150px'}} fontSize={14}
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <SocksContext.Provider value={product}>
                        <Socks key={product._id} product={product} />
                      </SocksContext.Provider>
                    </Suspense>
                  ))
                ) : (
                  <Text textAlign={'center'}>No products found in this category.</Text>
                )}
              </Box>
