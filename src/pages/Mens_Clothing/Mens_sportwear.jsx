...
export const SportwearContext = createContext();

const Sportwear = React.lazy(() => import('../../components/mensCategory/Sportwear'));

// Main Component
export default function Mens_sportwear() {
  const clothingLinks = [
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <SportwearContext.Provider value={product}>
                        <Sportwear key={product._id} product={product} />
                      </SportwearContext.Provider>
                    </Suspense>
                  ))
                ) : (
