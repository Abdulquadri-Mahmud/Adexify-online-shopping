...
const Pants = React.lazy(() => import('../../components/mensCategory/Pants'));

// Main Component
export default function Pants_page() {
  const clothingLinks = [
  ...
  const filteredProducts = products.filter((product) => {
    ...
    filter === 'All' ? product.gender === 'male' && product.category === 'Pants' : product.category === filter && product.gender === 'male';
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <PantsContext.Provider value={product}>
                        <Pants key={product._id} product={product} />
