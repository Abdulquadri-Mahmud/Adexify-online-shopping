...
const Shirts = React.lazy(() => import('../../components/mensCategory/Shirts'));

// Main Component
export default function Shirts_page() {
  const clothingLinks = [
  ...
  const filteredProducts = products.filter((product) => {
    ...
    filter === 'All' ? product.gender === 'male' && product.category === 'Shirt' : product.category === filter && product.gender === 'male';
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <ShirtsContext.Provider value={product}>
                        <Shirts key={product._id} product={product} />
