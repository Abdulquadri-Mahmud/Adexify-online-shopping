...
export const Hoodies_SweatshirtContext = createContext();

const Hoodies_Sweatshirt = React.lazy(() => import('../../components/mensCategory/Hoodies_Sweatshirt'));

// Main Component
export default function Hoodies_sweatshirt_page() {
  const clothingLinks = [
  ...
  const filteredProducts = products.filter((product) => {
    ...
    filter === 'All' ? product.gender === 'male' && product.category === 'Hoodies & Sweatershirt' : product.category === filter && product.gender === 'male';
                ...
                {currentProducts.length > 0 ? (
                  currentProducts.map((product) => (
                    <Suspense fallback={<Loading/>}>
                      <Hoodies_SweatshirtContext.Provider value={product}>
                        <Hoodies_Sweatshirt key={product._id} product={product} />
                      </Hoodies_SweatshirtContext.Provider>
                    </Suspense>
                  ))
                ) : (
