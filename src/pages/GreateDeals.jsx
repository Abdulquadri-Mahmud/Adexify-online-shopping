  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const greatDeals = data.filter((product) => product.deal === "great");
        setProducts(greatDeals);
        setFilteredProducts(greatDeals);
        setLoading(false);
      } catch (error) {
    ...
    <Box className="bg-zinc-200">
      ...
      <Box px={{ base: 4, md: 10 }} pt={10}>
        ...
        <Box bgGradient="linear(to-r, green.500, green.300)" color="white" textAlign="center" py={12} borderRadius="lg" mb={10}>
          <Heading size="2xl" mb={2}>Fashion Deals You Can’t Resist</Heading>
          <Text fontSize="lg">Top fashion products at amazing prices!</Text>
        </Box>
        ...
        <Box py={4} bg={'white'} rounded={'md'} maxW={'md'} mx={'auto'} mb={6} px={4}>
          <InputGroup maxW="lg" mx="auto">
            ...
            <Input type="text" placeholder="Search fashion deals" value={search} onChange={handleSearch} bg="white" />
          </InputGroup>
        </Box>
